...
3. **Copy the remote device folder to the Pi**
   ...
   ```
   Replace `<pi-ip>` with the Pi's address; use the username you set in Imager (Bookworm requires you to create one). `~/` resolves to that user's home directory (e.g., `/home/homebrain`).
