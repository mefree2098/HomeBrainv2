...
Run the helper script once (and again after future upgrades) to provision the Python toolchain (PyTorch + torchaudio/torchmetrics/torchinfo, soundfile, librosa, pronouncing, audiomentations + torch-audiomentations, webrtcvad, speechbrain, mutagen, onnxruntime/onnx-tf, TensorFlow Lite converters, and openwakeword):
