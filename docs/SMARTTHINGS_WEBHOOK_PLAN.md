...
1. **Environment Variables**:
   ...
   - `SMARTTHINGS_WEBHOOK_METRICS_INTERVAL_MS` controls automatic metrics snapshotting (defaults to 60 s, set to `0` to disable).
   - `SMARTTHINGS_WEBHOOK_METRICS_HISTORY` limits retained in-memory history entries (defaults to 1440 snapshots).
...
2. **Production Validation Window**:
   - Coordinate a short production window to reinstall/authorize the SmartApp.
...
- Runtime adjustments: `POST /api/smartthings/webhook/metrics/config` (auth required) to set production cadence (e.g., `intervalMs=300000`, `historySize=288`) without redeploying.
...
1. Schedule the production cutover window: reinstall/authorize the SmartApp, validate lifecycle handling live, and capture metrics snapshots.
2. Extend deployment/runbook documentation to cover the production rollout, fallback switches, and alert response procedures (including the new environment variables).
3. Integrate the webhook metrics endpoint into dashboards/alerting and verify the automated subscription renewal job against production traffic.
...
- **2025-10-18** - Updated rollout plan for direct-to-production deployment: testing plan now focuses on coordinated production validation and post-cutover monitoring.
...
- **2025-10-18** - Implemented in-process metrics sampling/history (`SMARTTHINGS_WEBHOOK_METRICS_INTERVAL_MS`) and exposed `GET /api/smartthings/webhook/metrics/history` for quick production checks without an external monitoring stack.
- **2025-10-18** - Added authenticated `POST /api/smartthings/webhook/metrics/config` to adjust interval/history at runtime, enabling production cadence tuning without redeploying.
