...
| 2026-05-17 11:32 MDT | Deploy gate repair | The final tracker deploy surfaced one remaining Sense backoff test assumption: the scheduler stores remaining backoff time, so a millisecond of elapsed time could make the timer interval just under the configured base. The test now verifies the applied backoff value and allows sub-second elapsed-time drift. | Pass locally | Re-run production safe deploy. |
