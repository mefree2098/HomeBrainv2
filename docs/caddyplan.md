...
2. Keep `mail.example.com` in DNS now; the route will already be seeded disabled, so only enable it once the Axiom service is actually present on its internal upstream.
