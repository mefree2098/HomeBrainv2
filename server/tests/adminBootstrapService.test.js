...
test('ensureBootstrapState promotes and reactivates the configured admin account', async (t) => {
  ...
  process.env.HOMEBRAIN_DEFAULT_ADMIN_EMAIL = 'admin@example.com';
  process.env.HOMEBRAIN_DEFAULT_ADMIN_NAME = 'HomeBrain Admin';
  ...
  assert.equal(user.name, 'HomeBrain Admin');
...
test('ensureBootstrapState creates the configured admin when password bootstrap is available', async (t) => {
  ...
  assert.equal(createdPayload.name, 'HomeBrain Admin');
