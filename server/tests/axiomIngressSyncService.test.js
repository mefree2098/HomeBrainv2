...
function createRouteDocument(overrides = {}) {
  return {
    ...
    hostname: overrides.hostname || 'mail.example.com',
...
test('sync reconciles managed Axiom routes, redirect URIs, and applies the reverse-proxy config', async (t) => {
  ...
  global.fetch = async () => ({
    ...
    async json() {
      return {
        ...
        mailHosts: ['mail.example.com', 'mail.example.net'],
        homeBrainRedirectUris: [
          'https://mail.example.com/api/identity/homebrain/callback',
          'https://mail.example.net/api/identity/homebrain/callback'
        ]
      };
    }
  });
  ...
  reverseProxyService.applyConfig = async (actor) => {
    ...
    return {
      ...
      appliedRoutes: ['mail.example.com', 'mail.example.net']
    };
  };
  ...
  assert.equal(createdRoutes[0].payload.hostname, 'mail.example.net');
  ...
  assert.equal(updatedRoutes[0].payload.hostname, 'mail.example.com');
  ...
  assert.deepEqual(result.routes.created, ['mail.example.net']);
  assert.deepEqual(result.routes.updated, ['mail.example.com']);
...
test('sync creates the default Axiom OIDC client when bootstrap has not seeded it yet', async (t) => {
  ...
  global.fetch = async () => ({
    ...
    async json() {
      return {
        ...
        mailHosts: ['mail.example.com'],
        homeBrainRedirectUris: ['https://mail.example.com/api/identity/homebrain/callback']
      };
    }
  });
  ...
  reverseProxyService.applyConfig = async () => ({
    ...
    appliedRoutes: ['mail.example.com']
  });
  oidcService.ensureBootstrapState = async () => ({
