...
test('direct radio refresh preserves known-good Z-Wave siren runtime during startup dead sync', () => {
  const existing = {
    name: 'Kitchen Siren',
    type: 'siren',
    room: 'Upstairs',
  ...
  const update = {
    ...
    properties: {
      ...
      homebrainDirect: {
        ...
        isListening: true,
        lastReason: 'sync',
        lastSeen: '2026-06-02T00:47:00.000Z',
        catalog: {
          manufacturer: 'AEON Labs',
          label: 'ZW080'
        }
  ...
  assert.equal(merged.properties.homebrainDirect.interviewStage, '5');
  assert.equal(merged.properties.homebrainDirect.lastPartialReason, 'sync');
  assert.equal(merged.properties.homebrainDirect.lastPartialAt, '2026-06-02T00:47:00.000Z');
  assert.deepEqual(merged.properties.directRadioFeatures, ['alarm', 'button', 'switch']);
  assert.equal(merged.properties.supportsAlarm, true);
  assert.equal(merged.properties.supportsSirenVolume, true);
  assert.equal(merged.properties.supportsSirenSound, true);
...
test('pending recovered Zigbee contact migrations finalize when native IAS and contact state become ready', async () => {
  const service = createService();
  const pendingDevice = {
    _id: DEVICE_ID,
    name: 'Back Door',
    ...
    properties: {
      ...
      directRadioFeatures: ['battery', 'contact', 'temperature', 'vibration'],
      directRadioState: {
        contactOpen: false,
        contact: 'closed',
        batteryLevel: 33
      ...
      smartThingsMigration: {
        status: 'native_joined_pending_interview',
        migrationId: 'migration-back-door',
        recoveredAt: '2026-05-31T23:34:26.447Z'
      }
    }
  };
  let finalizationArgs = null;
  service.finalizeDeviceMigration = async (args) => {
    finalizationArgs = args;
    return {
      device: {
        ...pendingDevice,
        properties: {
          ...pendingDevice.properties,
          smartThingsMigration: {
            ...pendingDevice.properties.smartThingsMigration,
            finalizedAt: '2026-05-31T23:59:00.000Z'
          }
        }
      }
    };
  ...
  const result = await service.finalizePendingSmartThingsMigrationIfReady(pendingDevice, {
    protocol: 'zigbee',
    id: '0x000d6f00057c378b'
  });

  assert.equal(finalizationArgs.deviceId, DEVICE_ID);
  assert.equal(finalizationArgs.migrationId, 'migration-back-door');
  assert.equal(result.properties.smartThingsMigration.finalizedAt, '2026-05-31T23:59:00.000Z');
});
