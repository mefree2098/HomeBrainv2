...
const voiceCommandSchema = new mongoose.Schema({
  ...
  intent: {
    action: {
      type: String,
      required: true,
      enum: ['device_control', 'scene_activate', 'automation_create', 'automation_control', 'workflow_create', 'workflow_control', 'workflow_revise', 'query', 'system_control', 'unknown']
    },
    confidence: {
      type: Number,
      min: 0,
  ...
  execution: {
    ...
    actions: [{
      type: {
        type: String,
        enum: ['device_control', 'scene_activate', 'automation_create', 'automation_trigger', 'workflow_create', 'workflow_control', 'workflow_revise', 'tts_response', 'notification']
      },
      target: String,
