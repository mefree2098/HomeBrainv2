...
WAIT_SECONDS="${HOMEBRAIN_RESTORE_WAIT_SECONDS:-20}"
...
if [[ -z "${NODE_BIN}" ]]; then
  echo "node is not available on this host." >&2
  exit 1
fi

if [[ -z "${HOMEBRAIN_DIR}" ]]; then
  echo "HOMEBRAIN_DIR must be set for the restore helper." >&2
  exit 1
fi
...
cleanup_orphaned_homebrain_processes() {
  ...
  service_pid="$(sudo "${SYSTEMCTL_BIN}" show -p MainPID --value "${SERVICE_NAME}" 2>/dev/null || echo 0)"
...
get_service_state() {
  sudo "${SYSTEMCTL_BIN}" show -p ActiveState --value "${SERVICE_NAME}" 2>/dev/null || true
}

get_service_main_pid() {
  local main_pid
  main_pid="$(sudo "${SYSTEMCTL_BIN}" show -p MainPID --value "${SERVICE_NAME}" 2>/dev/null || echo 0)"
  if [[ -z "${main_pid}" ]]; then
    echo 0
    return
  fi
...
cleanup_blocking_homebrain_port_listeners() {
  ...
  sleep 2
  kill -9 "${blocking_pids[@]}" 2>/dev/null || true
}
...
stop_homebrain_service() {
  ...
  sudo "${SYSTEMCTL_BIN}" stop "${SERVICE_NAME}" --no-block || true
  ...
  while true; do
    ...
    if (( elapsed >= WAIT_SECONDS )); then
      ...
      sudo "${SYSTEMCTL_BIN}" kill --kill-who=all --signal=SIGKILL "${SERVICE_NAME}" 2>/dev/null || true
      sudo "${SYSTEMCTL_BIN}" stop "${SERVICE_NAME}" >/dev/null 2>&1 || true
      break
    fi
...
attempt_service_start() {
  ...
  sudo "${SYSTEMCTL_BIN}" daemon-reload || true
  sudo "${SYSTEMCTL_BIN}" start "${SERVICE_NAME}"
}

run_restore_job() {
  cd "${HOMEBRAIN_DIR}"
  "${NODE_BIN}" server/scripts/runQueuedRestoreJob.js --latest
}

echo "Preparing HomeBrain disaster recovery restore..."
sudo "${SYSTEMCTL_BIN}" start mongod
stop_homebrain_service

if run_restore_job; then
  echo "Restore applied successfully."
  attempt_service_start
  echo "HomeBrain disaster recovery restore finished."
  exit 0
fi

echo "Restore failed. Attempting to bring ${SERVICE_NAME} back online..." >&2
